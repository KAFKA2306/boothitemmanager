...
def test_build_proof_counts_actual_records_and_separates_build_time(tmp_path: Path):
    api = tmp_path / "api"
    api.mkdir()
    (api / "catalog_summary_part1.json").write_text(
        json.dumps([{"id": "1"}, {"id": "2"}]), encoding="utf-8"
    )
    (api / "catalog_summary_part2.json").write_text(json.dumps([{"id": "3"}]), encoding="utf-8")
    (api / "metadata.json").write_text(
        json.dumps({"updated_at": "2026-08-20T00:00:00Z"}), encoding="utf-8"
    )
    (api / "freshness.json").write_text(
        json.dumps(
            {"last_catalog_change_at": "2026-08-20T01:00:00Z", "source": "https://booth.pm"}
        ),
        encoding="utf-8",
    )
