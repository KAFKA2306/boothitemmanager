...
def test_stable_refresh_batch_rotates_between_slots():
    ids = [str(i) for i in range(1, 1001)]
    a = stable_refresh_batch(
        ids, now=datetime(2026, 8, 21, 0, 7, tzinfo=timezone.utc), batch_size=120
    )
    b = stable_refresh_batch(
        ids, now=datetime(2026, 8, 21, 0, 22, tzinfo=timezone.utc), batch_size=120
    )
    assert a != b
