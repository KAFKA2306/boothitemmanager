...
def concept_invention_phase() -> None:
    ...
    for tag, items in extracted_map.items():
        ...
        pending_list.append({"tag": tag, "confidence": confidence, "sources": items})

    pending_list.sort(key=lambda x: (-x["confidence"], x["tag"]))
    StagingBuffer.save_pending_evolution(pending_list)
    print(
        f"✅ [EOL:CONCEPT_INVENTION] Found {len(pending_list)} new potential tags and buffered them."
    )
...
def validation_phase():
        ...
        json.dumps(
            {
                ...
                "description": "Enforcing API Schema Contract and Quality Gateway",
            }
        )
    )
    ...
    for item in catalog:
        ...
        if not tag_set.get("style") or not tag_set.get("color"):
            raise ValueError(
                json.dumps({"event": "quality_gate_rejected", "item_id": item.get("item_id")})
            )
        ...
        try:
            ...
            print(
                json.dumps(
                    {"event": "schema_violation", "item_id": item.get("item_id"), "error": str(e)}
                )
            )
            raise e
...
def promote_tags_phase():
    ...
    for entry in pending:
        ...
        if stem not in clusters:
            ...
            if entry["confidence"] > clusters[stem]["confidence"]:
                ...
                clusters[stem]["canonical_name"] = (
                    cleaned_tag  # Use the casing of the highest confidence match
                )

    # Stage 1: Frequency Filtering (sources must be at least 3 items)
    # Drop clusters appearing in < 3 items
    filtered_clusters = {stem: info for stem, info in clusters.items() if len(info["sources"]) >= 3}
    ...
    for stem, info in filtered_clusters.items():
        ...
        BAD_TAG_PATTERN = re.compile(
            r"^[0-9]+(アバター|avatars|avatar|人|モデル|対応| Avatars| Avatar)", re.I
        )
        GENERIC_EXCLUSIONS = {
            "3dモデル",
            "3d",
        ...
        is_style = (
            any(k in name_lower for k in style_keywords)
            or name_lower.endswith("style")
            or name_lower.endswith("kei")
        )
        ...
        if is_style:
            ...
            log_evolution(
                "style_promoted",
                f"Promoted tag '{name}' to styles.yaml with confidence {info['confidence']:.2f}",
            )
        else:
            # tags.yaml promotion
            section = "accessories"
            ...
            if matched_key:
                ...
                tags_data[section][name] = {
                    ...
                    "metadata": {"faceemo_version": "any", "udonsharp_compat": True},
                }
            promoted_tags += 1
            log_evolution(
                "tag_promoted",
                f"Promoted tag '{name}' to tags.yaml ({section}) with confidence {info['confidence']:.2f}",
            )
    ...
    print(
        f"✅ [EOL:PROMOTION_ENGINE] Completed promotion. Promoted tags: {promoted_tags}, Promoted styles: {promoted_styles}"
    )
...
def pruning_pipeline_phase() -> None:
    ...
    if len(active_tags) < 5:
        print(
            "⚠️ [EOL:PRUNING_PIPELINE] Too few active tags found via date window. Falling back to entire catalog presence."
        )
        for item in catalog:
            tag_set = item.get("tag_set") or {}
            for t_list in tag_set.values():
    ...
    for section_name, section_dict in list(tags_data.items()):
        ...
        for tag_key, tag_info in list(section_dict.items()):
            if not isinstance(tag_info, dict):
                continue
            aliases = tag_info.get("aliases") or []
            is_active = tag_key.lower() in active_tags or any(
                a.lower() in active_tags for a in aliases
            )
            if not is_active:
                best_match = None
                best_ratio = 0.0
                ...
                if best_match:
                    ...
                    log_evolution(
                        "tag_merged",
                        f"Merged inactive tag '{tag_key}' into active tag '{best_match}'",
                    )
                    tags_modified = True
                else:
    ...
    for style_key, aliases in list(style_dict.items()):
        is_active = style_key.lower() in active_tags or any(
            a.lower() in active_tags for a in aliases
        )
        if not is_active:
            best_match = None
            ...
            if best_match:
                ...
                log_evolution(
                    "style_merged",
                    f"Merged inactive style '{style_key}' into active '{best_match}'",
                )
                styles_modified = True
            else:
                styles_data.setdefault("style_metadata", {}).setdefault(style_key, {})["status"] = (
                    "deprecated"
                )
        ...
        json.dumps(
            {
                ...
                "description": "Initializing Autonomous Evolution Loop (Zero-Trust Mode)",
            }
        )
    )
    try:
        ...
        json.dumps(
            {
                ...
                "description": "Cycle Complete. Axiomatic Space remains consistent.",
            }
        )
    )
