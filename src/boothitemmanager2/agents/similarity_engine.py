...
def calculate_similar_items(items: List[Item], trace_id: str='default') -> TestBlock:
    if not items:
        return TestBlock(trace_id=trace_id, input=0, pre_state={}, action='calculate_similarity', expected_state={'item_count': 0}, actual_state={'items': [], 'item_count': 0}, diff={}, result='SUCCESS')
    ...
    return TestBlock(trace_id=trace_id, input={'item_count': len(items)}, pre_state={}, action='calculate_similar_items', expected_state={'processed_count': len(items)}, actual_state={'items': updated_items, 'processed_count': len(updated_items)}, diff={}, result='SUCCESS')