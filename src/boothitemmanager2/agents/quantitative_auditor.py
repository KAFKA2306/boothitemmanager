"""
BoothItemManager2 Quantitative Audit Agent
===================================
入力データのみを根拠とする定量監査エンジン。
推測・外部補完・テキスト説明は一切禁止。
数値構造のみを出力する。
"""
...
_TOTAL_FEATURES = 4  # color / style / avatar_reverse / cross_category
...
class AuditReport:
    ...
    # ── 3. クロール整合性 ────────────────────────────────
    source_items: int          # 0 = Unknown
    indexed_items: int
    lost_items: int
    sync_rate: float           # 0.0 = Unknown
...
def _has_metadata(item: Item) -> bool:
    return all(
        bool(getattr(item, f, None))
        for f in _REQUIRED_FIELDS
    )


def _count_invalid_tags(item: Item) -> int:
    """空文字・空白のみのタグを invalid とみなす。"""
...
def _has_multi_label_conflict(item: Item) -> bool:
    """同一アバターへの重複 AvatarRef が存在する場合にコンフリクトとみなす。"""
...
class QuantitativeAuditor:
    """
    BoothItemManager2定量監査エンジン。
    推測・補完・テキスト説明：禁止。
    入力スコープ：Item リスト + CrawlStats + FilterEvidence + FeatureStats のみ。
    """

    def run(
        self,
        items: Sequence[Item],
        crawl: CrawlStats = CrawlStats(),
        filters: FilterEvidence = FilterEvidence(),
        features: FeatureStats = FeatureStats(),
    ) -> AuditReport:
        # ── 1. データカバレッジ ─────────────────────────────
        total = len(items)
