...
def generate_api(items: List[Item], graph_data: Dict[str, Any], trace_id: str) -> TestBlock:
    ...
    with open(catalog_path, 'w', encoding='utf-8') as f:
        json.dump([asdict(item) for item in items], f, ensure_ascii=False, indent=2, default=serialize)
    summaries = []
    for item in items:
        summaries.append({'item_id': item.item_id, 'name': item.title, 'shop_name': item.creator_name, 'type': item.category.value, 'image_url': item.thumbnail_url, 'current_price': item.price, 'like_count': item.like_count})
    ...
    metrics = {'total_items': len(items), 'types': dict(type_counts), 'top_shops': dict(shop_counts.most_common(10)), 'top_avatars': dict(avatar_counts.most_common(10)), 'updated_at': datetime.now().isoformat()}
    metrics_path = os.path.join(api_dir, 'metrics.json')
    with open(metrics_path, 'w', encoding='utf-8') as f:
        json.dump(metrics, f, ensure_ascii=False, indent=2)
    return TestBlock(trace_id=trace_id, input=len(items), pre_state={}, action='generate_api', expected_state={'file_count_min': 3}, actual_state={'item_count': len(items), 'api_dir': api_dir}, diff={}, result='SUCCESS')