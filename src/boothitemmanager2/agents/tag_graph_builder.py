...
try:
    from ..schemas.storage import TagEdge, TagGraph, TagNode
except ImportError:
    import sys

    sys.path.append(str(Path(__file__).parent.parent.parent))
    from boothitemmanager2.schemas.storage import TagEdge, TagGraph, TagNode
...
class TagGraphBuilder:
    ...
    def build(self):
        ...
        nodes = [
            TagNode(tag_id=tid, name=tid, dimension=tag_to_dim.get(tid, "general"), weight=count)
            for (tid, count) in nodes_dict.items()
        ]
