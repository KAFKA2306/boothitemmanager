...
def normalize_html(raw_page: Any, trace_id: str) -> TestBlock:
    ...
    item = Item(item_id=item_id, source_url=raw_page.url, title=title, description=description, thumbnail_url=thumbnail_url or '', creator_id=creator_id or 'unknown', creator_name=creator_name, published_at=published_at, like_count=like_count, price=price, category=category, tag_set=tag_set, similar_items=[], user_state={}, tags_raw=tags_raw, targets=targets, files=files)
    return TestBlock(trace_id=trace_id, input=raw_page.url, pre_state={'url': raw_page.url}, action='normalize_html', expected_state={'item_id': item_id}, actual_state={'item': item}, diff={}, result='SUCCESS')
...
def extract_tag_set(name: str, description: str, tags: List[str], targets: List[AvatarRef], aliases: Dict[str, Any]) -> TagSet:
    ...
    res = {'appearance': [], 'body_type': [], 'style': [], 'color': [], 'outfit_type': [], 'accessory': [], 'feature': [], 'platform': [], 'season': [], 'avatar_link': [t.code for t in targets]}
    mapping = {'appearance_tags': 'appearance', 'mood_tags': 'style', 'color_tags': 'color', 'outfit_tags': 'outfit_type'}
