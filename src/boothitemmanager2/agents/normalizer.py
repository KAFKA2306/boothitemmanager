...
def normalize_html(raw_page: Any, trace_id: str) -> TestBlock:
    """
    Normalizes HTML content into the new 10D Item model.
    Crash-Driven: Ensures mandatory fields are present.
    """
...
def _parse_og_tags(soup: BeautifulSoup) -> Dict[str, str]:
    ...
    for tag in soup.find_all('meta', property=lambda x: x and x.startswith('og:')):
        ...
        if prop and content: og_data[prop] = content
    return og_data
...
def _pick_name(soup: BeautifulSoup, og_data: Dict[str, str]) -> str:
    val = og_data.get('title') or (soup.select_one('h1.item-name') or soup.select_one('h1')).get_text(strip=True)
    if not val: raise ValueError("Name extraction failed")
    return val.strip()
...
def _pick_image(soup: BeautifulSoup, og_data: Dict[str, str], base_url: str) -> Optional[str]:
    ...
    if img: return re.sub(r'/c/\d+x\d+/', '/', urljoin(base_url, img))
    return None
...
def _pick_tags(soup: BeautifulSoup) -> List[str]:
    return list(set(t.get_text(strip=True).replace('#', '') for t in soup.select('a.l-tag, a.u-tpg-label, .item-tags a')))
...
def pick_targets(name: str, description: str, tags: List[str], aliases: Dict[str, Any]) -> List[AvatarRef]:
    targets = []
    text = f"{name} {description} {' '.join(tags)}".lower()
    for code, data in aliases.get('avatars', {}).items():
        terms = [code.lower()] + [t.lower() for t in data.get('aliases', []) + [data.get('name_ja', ''), data.get('name_en', '')] if t]
        if any(t in text for t in terms):
            targets.append(AvatarRef(code=code, name=data.get('name_ja', code)))
    return targets

def infer_category(name: str, description: str, tags: List[str], targets: List[AvatarRef], aliases: Dict[str, Any]) -> ItemCategory:
    text = f"{name} {description} {' '.join(tags)}".lower()
    
    # Priority rules
    if any(k in text for k in ["vroid", "vroidhub"]): return ItemCategory.VROID
    if any(k in text for k in ["アニメーション", "motion", "モーション", "animation"]): return ItemCategory.ANIMATION
    if any(k in text for k in ["髪型", "ヘアスタイル", "hairstyle", "髪"]): return ItemCategory.HAIRSTYLE
    if any(k in text for k in ["アバター", "avatar"]) and "対応" not in name: return ItemCategory.AVATAR
    if any(k in text for k in ["衣装", "服", "outfit", "costume", "dress"]): return ItemCategory.OUTFIT
    if any(k in text for k in ["アクセ", "accessory", "靴", "メガネ", "帽子"]): return ItemCategory.ACCESSORY
    if any(k in text for k in ["テクスチャ", "texture", "瞳", "肌", "skin"]): return ItemCategory.TEXTURE
    if any(k in text for k in ["ワールド", "world", "scene", "シーン"]): return ItemCategory.WORLD
    if any(k in text for k in ["ギミック", "gimmick", "modular", "ma対応", "tool", "ツール"]): return ItemCategory.GIMMICK_TOOL
    if any(k in text for k in ["小道具", "prop", "家具", "椅子", "机"]): return ItemCategory.PROP
    
    return ItemCategory.ASSET

def extract_tag_set(name: str, description: str, tags: List[str], targets: List[AvatarRef], aliases: Dict[str, Any]) -> TagSet:
    def _norm(t: str) -> str: return unicodedata.normalize('NFKC', t).lower()
    text = _norm(f"{name} {description} {' '.join(tags)}")
    
    res = {
        "appearance": [], "body_type": [], "style": [], "color": [],
        "outfit_type": [], "accessory": [], "feature": [],
        "platform": [], "season": [], "avatar_link": [t.code for t in targets]
    }
    ...
    for section, dim in mapping.items():
        for key, data in aliases.get(section, {}).items():
            ...
            terms = [_norm(t) for t in data.get('aliases', []) + ([name_ja] if name_ja else []) if t]
            if any(t in text for t in terms):
    ...
    for f, keywords in features.items():
        if any(k in text for k in keywords): res["feature"].append(f)
