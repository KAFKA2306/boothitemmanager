...
class AuditEngine:
    ...
    def evaluate_category(trace_id: str, category: str, items: List[AuditItem]) -> AuditChecklistResult:
        ...
        if final_status == 'FAIL':
            reject_reason = f"Audit failed in category '{category}': " + '; '.join([f'{i.description} ({i.message})' for i in failed_items])
        return AuditChecklistResult(trace_id=trace_id, category=category, items=items, final_status=final_status, reject_reason=reject_reason)
