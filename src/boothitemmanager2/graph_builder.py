...
from .storage import GraphEdge, GraphNode, Item
...
def build_graph(items: list[Item], trace_id: str) -> TestBlock:
    ...
    for item in items:
        ...
        if creator_node_id not in nodes:
            nodes[creator_node_id] = GraphNode(
                node_id=creator_node_id, node_type="creator", edges=[]
            )
        edges_for_item: list[GraphEdge] = [
    ...
    nodes_out = [
        {"id": n.node_id, "label": n.node_type.capitalize(), "properties": {"name": n.node_id}}
        for n in nodes.values()
    ]
    edges_out = [
        {"source": src, "target": e.target_id, "relation": e.relation.upper(), "properties": {}}
        for (src, elist) in item_edges.items()
        for e in elist
    ]
    ...
    return TestBlock(
        ...
        actual_state={"node_count": len(nodes), "edge_count": len(edges_out)},
        diff={},
        result="SUCCESS",
    )
