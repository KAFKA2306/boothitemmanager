...
def validate_crawl(block: TestBlock) -> Message:
    ...
    payload = {'status': 'VALID' if is_valid else 'INVALID', 'observations': observations, 'metrics': {'status_code': actual.get('status_code'), 'content_length': actual.get('content_length')}}
    return Message(from_agent='crawler_validator', to_agent='main_agent', trace_id=block.trace_id, payload=payload)