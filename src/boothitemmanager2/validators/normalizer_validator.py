...
def validate_normalization(block: TestBlock) -> Message:
    ...
    payload = {'status': 'VALID' if is_valid else 'INVALID', 'reasons': reasons, 'observations': observations, 'item_summary': {'item_id': item.item_id if item else None, 'name': item.name if item else None, 'type': item.type if item else None, 'shop_name': item.shop_name if item else None, 'creator_id': item.creator_id if item else None} if item else None}
    return Message(from_agent='normalizer_validator', to_agent='main_agent', trace_id=block.trace_id, payload=payload)