...
def validate_graph(block: TestBlock) -> Message:
    """
    Validates the Graph build result by checking nodes and edges files.
    No REJECT authority.
    """
    ...
    is_valid = (nodes_exist and edges_exist and 
                node_count == expected_nodes and 
    ...
    payload = {
        "status": status,
        "nodes_verified": nodes_exist,
        "edges_verified": edges_exist,
        "counts": {
            "nodes": {"expected": expected_nodes, "actual": node_count},
            "edges": {"expected": expected_edges, "actual": edge_count}
        }
    }

    return Message(
        from_agent="graph_validator",
        to_agent="main_agent",
