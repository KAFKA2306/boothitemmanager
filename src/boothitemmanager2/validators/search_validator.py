...
def validate_search_index(block: TestBlock) -> Message:
    ...
    payload = {'status': status, 'file_exists': file_exists, 'is_valid_json': is_valid_json, 'expected_count': expected_count, 'actual_count': indexed_count, 'details': f'Verified {indexed_count} entries in {output_path}'}
    return Message(from_agent='search_validator', to_agent='main_agent', trace_id=block.trace_id, payload=payload)