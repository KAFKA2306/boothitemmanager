...
def validate_api(block: TestBlock) -> Message:
    ...
    if existence['metrics.json']:
        with open(os.path.join(api_dir, 'metrics.json'), 'r', encoding='utf-8') as f:
            metrics = json.load(f)
    ...
    payload = {'status': status, 'existence': existence, 'metrics_valid': metrics_valid, 'item_count': item_count, 'details': f'API validation complete for {api_dir}. Status: {status}'}
    return Message(from_agent='api_validator', to_agent='main_agent', trace_id=block.trace_id, payload=payload)