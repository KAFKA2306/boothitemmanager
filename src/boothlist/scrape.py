...
class BoothScraper:
    ...
    def _pick_creator_id(self, soup: BeautifulSoup, response_url: str) -> str | None:
        for selector in ["a.shop-name", "div.u-text-ellipsis > a", 'a[itemprop="author"]']:
            ...
            if elem:
                href = elem.get("href", "")
                if m := re.search(r"https://([^.]+)\.booth\.pm", href):
                    return m.group(1)
                if m := re.search(r"/shop/([^/?]+)", href):
                    return m.group(1)
    ...
    def _pick_price(
        self, soup: BeautifulSoup, og_data: dict[str, str], json_ld: dict[str, Any] | None = None
    ) -> int | None:
        if json_ld:
            offers = json_ld.get("offers")
            if offers:
                if isinstance(offers, dict):
                    if (price := offers.get("price")) is not None:
                        return int(float(str(price).replace(",", "")))
                    if (low_price := offers.get("lowPrice")) is not None:
                        return int(float(str(low_price).replace(",", "")))
                elif isinstance(offers, list) and len(offers) > 0:
                    if (price := offers[0].get("price")) is not None:
                        return int(float(str(price).replace(",", "")))
        if og_price := og_data.get("price:amount"):
            if m := re.search(r"[\d,]+", str(og_price)):
                return int(m.group().replace(",", ""))
        for selector in ["div.price", 'span[itemprop="price"]', ".price .yen", ".item-price .yen"]:
            elem = soup.select_one(selector)
            if elem:
                if m := re.search(r"¥\s*([\d,]+)", elem.get_text(strip=True)):
                    return int(m.group(1).replace(",", ""))
        return None

    def _pick_image(
        self, soup: BeautifulSoup, og_data: dict[str, str], base_url: str
    ) -> str | None:
        if og_data.get("image"):
            return self._normalize_image_quality(urljoin(base_url, og_data["image"]))
        return None

    def _pick_description(self, soup: BeautifulSoup, og_data: dict[str, str]) -> str | None:
        if og_data.get("description"):
            return og_data["description"].strip()[:200]
